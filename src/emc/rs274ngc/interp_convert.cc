                          ...
                          block_pointer block, setup_pointer settings)
{
    ...
    if (mode == G_5_2)
    {
        ...
        if (settings->plane == CANON_PLANE::XY)
        {
            ...
            CHKS((!(block->x_flag) && !(block->y_flag) && (block->p_number > 0) && (!nurbs_g5_control_points.empty())),
        ...
        if (settings->plane == CANON_PLANE::YZ)
        {
            ...
            CHKS((!(block->y_flag) && !(block->z_flag) && (block->p_number > 0) && (!nurbs_g5_control_points.empty())),
        ...
        if (settings->plane == CANON_PLANE::XZ)
        {
            ...
            CHKS((!(block->x_flag) && !(block->z_flag) && (block->p_number > 0) && (!nurbs_g5_control_points.empty())),
                 (_("Can specify P without X and Z only for the first control point")));
        }

        CHKS(((block->p_number <= 0) && (!nurbs_g5_control_points.empty())),
        ...
        if (settings->plane == CANON_PLANE::XY)
        {
            ...
            {
                CHP(find_ends(block, settings, &CP_G5.NURBS_X,
        ...
        if (settings->plane == CANON_PLANE::YZ)
        {
            ...
            {
                CHP(find_ends(block, settings, &end_x,
        ...
        if (settings->plane == CANON_PLANE::XZ)
        {
            ...
            {
                CHP(find_ends(block, settings, &CP_G5.NURBS_Y,
    ...
    else if (mode == G_5_3)
    {
        ...
        CHKS((nurbs_g5_control_points.size() < nurbs_order),
             _("You must specify a number of control points at least equal to the order L = %d"),
             nurbs_order);
        ...
        if (settings->plane == CANON_PLANE::XY)
        {
            settings->current_x =
                nurbs_g5_control_points
                    [nurbs_g5_control_points.size() - 1]
                        .NURBS_X;
            settings->current_y =
                nurbs_g5_control_points
                    [nurbs_g5_control_points.size() - 1]
    ...
    if (mode == G_6_2)
    {
        ...
        CHKS(((settings->plane != CANON_PLANE::XY) && (settings->plane != CANON_PLANE::YZ) && (settings->plane != CANON_PLANE::XZ)),
             (_("one plane must be selected for nurbs: XY, YZ, ZX")));
        ...
        CHKS(((!block->p_flag) && (nurbs_g6_control_points.empty())),
             (_("Program the nurbs order P in the first block of instruction")));
        int p;
        if (block->p_flag)
        {
        ...
        CHKS(((block->q_number <= 0) && (nurbs_g6_control_points.empty())),
             (_("Program the nurbs Q value (1,2,3) in the first block of nurbs instruction")));
        int q;
        if (block->q_flag)
        {
            CHKS((block->q_number < 1.0) || (block->q_number > 3.0),
                 (_("nurbs Q value not in range 1 to 3")));
            CHKS((!is_near_int(&q, block->q_number)),
                 _("nurbs Q number is not an integer"));
        }

        CHKS(((block->r_number <= 0) && (!nurbs_g6_control_points.empty())),
             (_("Must specify positive nurbs weight R for every Control Point")));

        CHKS(((block->k_number < 0) && (!nurbs_g6_control_points.empty())),
             (_("Must specify nurbs K number for every Control Point")));
        int k;
        if (block->k_flag)
        {
        ...
        if (settings->plane == CANON_PLANE::XY)
        {
            if (((block->x_flag) && (block->y_flag)) || block->k_flag)
            {
                CHP(find_ends(block, settings, &CP_G6.NURBS_X,
        ...
        if (settings->plane == CANON_PLANE::YZ)
        {
            if (((block->y_flag) && (block->z_flag)) || block->k_flag)
            {
                CHP(find_ends(block, settings, &end_x,
        ...
        if (settings->plane == CANON_PLANE::XZ)
        {
            if (((block->x_flag) && (block->z_flag)) || block->k_flag)
            {
                CHP(find_ends(block, settings, &CP_G6.NURBS_Y,
        ...
        if (nurbs_g6_control_points.size() > nurbs_order)
        {
                    ...
                    .NURBS_K)
            {
                CHKS((nurbs_g6_control_points.size() <
                      nurbs_order),
                     _("You must specify a number of control points at least equal to the order P = %d"),
                     nurbs_order);
                settings->current_x =
...
{ //!< pointer to machine settings
    ...
    if ((mode == G_5_1) || (mode == G_6_1))
    {
        ...
        if (settings->plane == CANON_PLANE::XY)
        {
            ...
            CHP(find_ends(block, settings, &x2, &y2, &end_z, &AA_end,
        ...
        if (settings->plane == CANON_PLANE::YZ)
        {
            ...
            CHP(find_ends(block, settings, &end_x, &y2, &z2, &AA_end,
                          &BB_end, &CC_end, &u_end, &v_end, &w_end));
        ...
        if (settings->plane == CANON_PLANE::XZ)
        {
            x1 = settings->current_x + block->i_number;
            z1 = settings->current_z + block->j_number;
            CHP(find_ends(block, settings, &x2, &end_y, &z2, &AA_end,
                          &BB_end, &CC_end, &u_end, &v_end, &w_end));
    ...
    { // !(mode == G_5_1)
        ...
        else
        {
            if (settings->plane == CANON_PLANE::XY)
            {
                x1 = settings->current_x + block->i_number;
                y1 = settings->current_y + block->j_number;
                CHP(find_ends(block, settings, &x3, &y3, &end_z,
                              &AA_end, &BB_end, &CC_end, &u_end,
                              &v_end, &w_end));
            ...
            if (settings->plane == CANON_PLANE::YZ)
            {
                y1 = settings->current_y + block->i_number;
                z1 = settings->current_z + block->j_number;
                CHP(find_ends(block, settings, &end_x, &y3, &z3,
                              &AA_end, &BB_end, &CC_end, &u_end,
                              &v_end, &w_end));
            ...
            if (settings->plane == CANON_PLANE::XZ)
            {
                x1 = settings->current_x + block->i_number;
                z1 = settings->current_z + block->j_number;
                CHP(find_ends(block, settings, &x3, &end_y, &z3,
                              &AA_end, &BB_end, &CC_end, &u_end,
                              &v_end, &w_end));
                        ...
                        setup_pointer settings) //!< pointer to machine settings
{
    ...
    CHKS((settings->plane == CANON_PLANE::UV || settings->plane == CANON_PLANE::VW || settings->plane == CANON_PLANE::UW),
                              ...
                              double u, double v, double w) //!< uvw at end of arc
{
    ...
    beta = (beta > (1.5 * M_PIl)) ? (beta - (2 * M_PIl)) : (beta < -M_PI_2l) ? (beta + (2 * M_PIl))
                                                                             : beta;
...
int Interp::convert_comment(char *comment, bool enqueue) //!< string with comment
{
    enum
    {
        LC_SIZE = 256,
        EX_SIZE = 2 * LC_SIZE
    }; // 256 from comment[256] in rs274ngc.hh
    char lc[LC_SIZE + 1];
                            ...
                            setup_pointer settings) //!< pointer to machine settings
{
    ...
    if (code == G_10)
    {
        ...
        else if (block->l_number == 0)
        {
            ...
            if (tno > 0)
            {
                ERS("G10 L0 not allowed with loaded tool <%d>\n", tno);
            }
                           ...
                           setup_pointer settings) //!< pointer to machine settings
{
    ...
    if (ai || bi || ci)
    {
        ...
        {
            anum = 3;
            jnum = settings->a_indexer_jnum;
        }
        else if (bi)
        {
            anum = 4;
            jnum = settings->b_indexer_jnum;
        }
        else if (ci)
        {
            anum = 5;
            jnum = settings->c_indexer_jnum;
        }
    ...
    else if ((motion == G_5) || (motion == G_5_1) || (motion == G_6) || (motion == G_6_1))
    { // jjf
        write_canon_state_tag(block, settings);
...
int Interp::convert_setup_tool(block_pointer block, setup_pointer settings)
{
    ...
    if (block->r_flag)
        settings->tool_table[idx].diameter = PROGRAM_TO_USER_LEN(block->r_number) * 2.;
    if (block->i_flag)
        settings->tool_table[idx].frontangle = block->i_number;
    if (block->j_flag)
        settings->tool_table[idx].backangle = block->j_number;
    ...
    // spindle)
    if (!_setup.random_toolchanger && idx == settings->current_pocket)
    {
                          ...
                          setup_pointer settings) //!< pointer to machine settings
{
    ...
    if (block->z_flag)
    {
        z = block->z_number;
        if (block->l_number == 20)
            z = cz + USER_TO_PROGRAM_LEN(parameters[5203 + (p_int * 20)]) - z;
    ...
    if (block->a_flag)
    {
        a = block->a_number;
        if (block->l_number == 20)
            a = ca + USER_TO_PROGRAM_ANG(parameters[5204 + (p_int * 20)]) - a;
    ...
    if (block->b_flag)
    {
        b = block->b_number;
        if (block->l_number == 20)
            b = cb + USER_TO_PROGRAM_ANG(parameters[5205 + (p_int * 20)]) - b;
    ...
    if (block->c_flag)
    {
        c = block->c_number;
        if (block->l_number == 20)
            c = cc + USER_TO_PROGRAM_ANG(parameters[5206 + (p_int * 20)]) - c;
    ...
    if (block->u_flag)
    {
        u = block->u_number;
        if (block->l_number == 20)
            u = cu + USER_TO_PROGRAM_LEN(parameters[5207 + (p_int * 20)]) - u;
    ...
    if (block->v_flag)
    {
        v = block->v_number;
        if (block->l_number == 20)
            v = cv + USER_TO_PROGRAM_LEN(parameters[5208 + (p_int * 20)]) - v;
    ...
    if (block->w_flag)
    {
        w = block->w_number;
        if (block->l_number == 20)
            w = cw + USER_TO_PROGRAM_LEN(parameters[5209 + (p_int * 20)]) - w;
                         ...
                         setup_pointer settings) //!< pointer to machine settings
{
    ...
    { /* reset stuff here */
        ...
        if (_setup.percent_flag && _setup.file_pointer)
        {
            ...
            for (;;)
            { /* check for ending percent sign and comment if missing */
...
{
    STRAIGHT_TRAVERSE(block->line_number, boring ? safe_x + depth - end_depth : safe_x - depth + end_depth,
                      start_y, start_z - zoff, AABBCC); // back
    if (taper_dist && entry_taper)
    {
        DISABLE_FEED_OVERRIDE();
        START_SPEED_FEED_SYNCH(settings->active_spindle, taper_pitch, 0);
        STRAIGHT_FEED(block->line_number, boring ? safe_x + depth - full_threadheight : safe_x - depth + full_threadheight,
                                   ...
                                   double u_end, double v_end, double w_end)
{
    ...
    else
    {
        ...
        (move == G_0 ? enqueue_STRAIGHT_TRAVERSE : enqueue_STRAIGHT_FEED)(settings, block->line_number,
                                                                          px - opx, py - opy, pz - opz,
                                                                          end_x, end_y, pz,
                                                                          AA_end, BB_end, CC_end,
                                                                          u_end, v_end, w_end);
                                       ...
                                       setup_pointer settings) //!< pointer to machine settings
{
    ...
    else if (g_code == G_43_1)
    {
        ...
        idx = -1;
        if (block->x_flag)
            tool_offset.tran.x = block->x_number;
        if (block->y_flag)
            tool_offset.tran.y = block->y_number;
        if (block->z_flag)
            tool_offset.tran.z = block->z_number;
        if (block->a_flag)
            tool_offset.a = block->a_number;
        if (block->b_flag)
            tool_offset.b = block->b_number;
        if (block->c_flag)
            tool_offset.c = block->c_number;
        if (block->u_flag)
            tool_offset.u = block->u_number;
        if (block->v_flag)
            tool_offset.v = block->v_number;
        if (block->w_flag)
            tool_offset.w = block->w_number;
    }
    else if (g_code == G_43_2)
    {
        CHKS((block->h_flag && (block->x_flag || block->y_flag || block->z_flag || block->a_flag || block->b_flag || block->c_flag || block->u_flag || block->v_flag || block->w_flag)), (_("G43.2: Can not have both H and axis words")));
        CHKS((!block->h_flag && !block->x_flag && !block->y_flag && !block->z_flag && !block->a_flag && !block->b_flag && !block->c_flag && !block->u_flag && !block->v_flag && !block->w_flag), (_("G43.2: No axes specified and H word missing")));
        tool_offset = settings->tool_offset;
        if (block->h_flag)
        {
            CHP((find_tool_index(settings, block->h_number, &idx)));
