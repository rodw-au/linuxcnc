...
*num_aio*:: Maximum number of analog IO pins is set by *EMCMOT_MAX_AIO*.
...
* `motion.interp.straight-heading` (float, out) +
  The XY plane heading of the current linear move in degrees. Measured counter-clockwise from the +X axis (0 to 360). This value is 0.0 during circular moves or when no XY motion occurs.

* `motion.interp.arc-radius` (float, out) +
  The radius of the current circular move. This value is 0.0 during linear moves.
